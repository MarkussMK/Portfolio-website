...
function showUnifiedUnlockNotification(levelTitle, unlockedPageHref, unlockedPageName) {
    ...
    // Show XP tracker as popup on mobile (add .expanded)
    const tracker = document.getElementById('character-tracker');
    let xpPopupActive = false;
    if (tracker && window.matchMedia('(max-width: 768px)').matches) {
        tracker.classList.add('expanded');
        xpPopupActive = true;
    }
...
/* Mobile XP Tracker Toggle Function (temporarily commented out to fix syntax) */

function setupMobileXPToggle() {
    // Mobile toggle is available even when leveling is disabled —
    // when disabled the button will open the preview popup instead of toggling live XP.
    ...
    toggleBtn.addEventListener('click', (e) => {
        ...
        // If leveling is disabled: prefer expanding the disabled "collapsible" message
        if (!isLevelingEnabled()) {
            const isCollapsible = tracker.classList.contains('collapsible');
            const isExpanded = tracker.classList.contains('expanded');

            if (isCollapsible) {
                // Expand/collapse the disabled-state message (do NOT award XP or change storage)
                if (isExpanded) {
                    // collapse
                    tracker.classList.remove('expanded');
                    tracker.classList.add('collapsed');
                    const collToggle = tracker.querySelector('.xp-collapsible-toggle');
                    if (collToggle) {
                        collToggle.setAttribute('aria-expanded', 'false');
                        collToggle.setAttribute('aria-label', 'Show leveling info');
                        const csvg = collToggle.querySelector('svg');
                        if (csvg) csvg.style.transform = 'rotate(0deg)';
                    }

                    // return toggle back to body and show open-eye icon
                    document.body.appendChild(toggleBtn);
                    toggleBtn.classList.remove('active');
                    toggleBtn.innerHTML = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M16.0001 12C16.0001 14.2091 14.2092 16 12.0001 16C9.79097 16 8.0001 14.2091 8.0001 12C8.0001 9.79086 9.79097 8 12.0001 8C14.2092 8 16.0001 9.79086 16.0001 12ZM14.0001 12C14.0001 13.1046 13.1047 14 12.0001 14C10.8955 14 10.0001 13.1046 10.0001 12C10.0001 10.8954 10.8955 10 12.0001 10C13.1047 10 14.0001 10.8954 14.0001 12Z" fill="white"/></svg>';
                } else {
                    // expand
                    ...
                    if (collToggle) {
                        ...
                        collToggle.setAttribute('aria-label', 'Hide leveling info');
                        const csvg = collToggle.querySelector('svg');
                        if (csvg) csvg.style.transform = 'rotate(180deg)';
                    }

                    // move toggle into the tracker (mobile CSS .active will position it relative to tracker)
                    tracker.appendChild(toggleBtn);
                    toggleBtn.classList.add('active');
                    toggleBtn.innerHTML = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M3.41431 3.00005C3.02379 2.60952 2.39062 2.60952 2.0001 3.00005C1.60958 3.39057 1.60958 4.02374 2.0001 4.41426L4.63205 7.04621C2.99403 8.55552 1.92448 10.2572 1.39435 11.2219C1.12571 11.7108 1.12571 12.2893 1.39435 12.7782C2.49915 14.7886 5.94677 20.0001 12.0001 20.0001C13.7147 20.0001 15.2202 19.5819 16.5227 18.9369L19.5859 22.0001C19.9764 22.3906 20.6096 22.3906 21.0001 22.0001C21.3906 21.6095 21.3906 20.9764 21.0001 20.5858L18.2746 17.8604M3.25071 12C3.75923 11.1073 4.69194 9.69405 6.04764 8.4618L8.23423 10.6484C8.08267 11.0706 8.0001 11.5257 8.0001 12C8.0001 14.2092 9.79096 16.0001 12.0001 16.0001C12.4745 16.0001 12.9295 15.9175 13.3518 15.7659L15.0097 17.4239C14.104 17.7849 13.1028 18.0001 12.0001 18.0001C9.59297 18.0001 7.66981 16.9749 6.16676 15.6451C4.74644 14.3885 3.77399 12.9187 3.25071 12Z" fill="white"/></svg>';
                }
                return;
            }

            // Fallback: show the read-only preview popup (used by Settings preview button)
            const isPreviewActive = tracker.dataset._previewActive === '1';
            if (!isPreviewActive) {
                previewDisabledPopup(true);
                tracker.dataset._previewActive = '1';
                toggleBtn.classList.add('active');
                toggleBtn.innerHTML = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M3.41431 3.00005C3.02379 2.60952 2.39062 2.60952 2.0001 3.00005C1.60958 3.39057 1.60958 4.02374 2.0001 4.41426L4.63205 7.04621C2.99403 8.55552 1.92448 10.2572 1.39435 11.2219C1.12571 11.7108 1.12571 12.2893 1.39435 12.7782C2.49915 14.7886 5.94677 20.0001 12.0001 20.0001C13.7147 20.0001 15.2202 19.5819 16.5227 18.9369L19.5859 22.0001C19.9764 22.3906 20.6096 22.3906 21.0001 22.0001C21.3906 21.6095 21.3906 20.9764 21.0001 20.5858L18.2746 17.8604M3.25071 12C3.75923 11.1073 4.69194 9.69405 6.04764 8.4618L8.23423 10.6484C8.08267 11.0706 8.0001 11.5257 8.0001 12C8.0001 14.2092 9.79096 16.0001 12.0001 16.0001C12.4745 16.0001 12.9295 15.9175 13.3518 15.7659L15.0097 17.4239C14.104 17.7849 13.1028 18.0001 12.0001 18.0001C9.59297 18.0001 7.66981 16.9749 6.16676 15.6451C4.74644 14.3885 3.77399 12.9187 3.25071 12Z" fill="white"/></svg>';
            } else {
                previewDisabledPopup(false);
                delete tracker.dataset._previewActive;
                toggleBtn.classList.remove('active');
                toggleBtn.innerHTML = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M16.0001 12C16.0001 14.2091 14.2092 16 12.0001 16C9.79097 16 8.0001 14.2091 8.0001 12C8.0001 9.79086 9.79097 8 12.0001 8C14.2092 8 16.0001 9.79086 16.0001 12ZM14.0001 12C14.0001 13.1046 13.1047 14 12.0001 14C10.8955 14 10.0001 13.1046 10.0001 12C10.0001 10.8954 10.8955 10 12.0001 10C13.1047 10 14.0001 10.8954 14.0001 12Z" fill="white"/></svg>';
            }
            return;
        ...
        // Leveling is enabled: toggle normally
    ...
    document.addEventListener('click', (e) => {
        ...
        if (tracker.dataset._previewActive === '1' && !tracker.contains(e.target) && !toggleBtn.contains(e.target)) {
            previewDisabledPopup(false);
            delete tracker.dataset._previewActive;
            toggleBtn.classList.remove('active');
            toggleBtn.innerHTML = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M16.0001 12C16.0001 14.2091 14.2092 16 12.0001 16C9.79097 16 8.0001 14.2091 8.0001 12C8.0001 9.79086 9.79097 8 12.0001 8C14.2092 8 16.0001 9.79086 16.0001 12ZM14.0001 12C14.0001 13.1046 13.1047 14 12.0001 14C10.8955 14 10.0001 13.1046 10.0001 12C10.0001 10.8954 10.8955 10 12.0001 10C13.1047 10 14.0001 10.8954 14.0001 12Z" fill="white"/></svg>';
        ...
        if (tracker.classList.contains('expanded') && 
            ...
            tracker.classList.remove('expanded');
            if (tracker.classList.contains('collapsible')) tracker.classList.add('collapsed');
            toggleBtn.classList.remove('active');
            document.body.appendChild(toggleBtn);
            toggleBtn.innerHTML = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M16.0001 12C16.0001 14.2091 14.2092 16 12.0001 16C9.79097 16 8.0001 14.2091 8.0001 12C8.0001 9.79086 9.79097 8 12.0001 8C14.2092 8 16.0001 9.79086 16.0001 12ZM14.0001 12C14.0001 13.1046 13.1047 14 12.0001 14C10.8955 14 10.0001 13.1046 10.0001 12C10.0001 10.8954 10.8955 10 12.0001 10C13.1047 10 14.0001 10.8954 14.0001 12Z" fill="white"/><path fill-rule="evenodd" clip-rule="evenodd" d="M22.6059 12.7781C22.8745 12.2893 22.8745 11.7107 22.6059 11.2219C21.5011 9.21142 18.0534 4 12.0001 4C5.94677 4 2.49915 9.21141 1.39435 11.2219C1.12571 11.7107 1.12571 12.2893 1.39435 12.7781C2.49915 14.7886 5.94677 20 12.0001 20C18.0534 20 21.5011 14.7886 22.6059 12.7781ZM20.7495 12C20.2262 11.0814 19.2538 9.61152 17.8334 8.35493C16.3304 7.02516 14.4072 6 12.0001 6C9.59298 6 7.66981 7.02516 6.16677 8.35493C4.74644 9.61152 3.77399 11.0814 3.25071 12C3.77399 12.9186 4.74644 14.3885 6.16677 15.6451C7.66981 16.9748 9.59298 18 12.0001 18C14.4072 18 16.3304 16.9748 17.8334 15.6451C19.2538 14.3885 20.2262 12.9186 20.7495 12Z" fill="white"/></svg>';
        }
...
function setupDesktopXPToggle() {
    // Desktop toggle available even when leveling is disabled — it will open the preview instead.
    ...
    toggleBtn.addEventListener('click', (e) => {
        ...
        // If leveling is disabled and tracker is the "collapsible" disabled message,
        // toggle the collapsed message (do NOT show live XP). Fall back to preview popup
        // only if the collapsible structure isn't present.
        if (!isLevelingEnabled()) {
            const isCollapsible = tracker.classList.contains('collapsible');
            const isExpanded = tracker.classList.contains('expanded');
            ...
            if (isCollapsible) {
                if (isExpanded) {
                    ...
                    toggleBtn.classList.remove('active');
                    // desktop stays fixed — show open-eye icon
                    toggleBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M16.0001 12C16.0001 14.2091 14.2092 16 12.0001 16C9.79097 16 8.0001 14.2091 8.0001 12C8.0001 9.79086 9.79097 8 12.0001 8C14.2092 8 16.0001 9.79086 16.0001 12ZM14.0001 12C14.0001 13.1046 13.1047 14 12.0001 14C10.8955 14 10.0001 13.1046 10.0001 12C10.0001 10.8954 10.8955 10 12.0001 10C13.1047 10 14.0001 10.8954 14.0001 12Z" fill="white"/></svg>';
                } else {
                    // expand the disabled message
                    tracker.classList.remove('collapsed');
                    tracker.classList.add('expanded');
                    const collToggle = tracker.querySelector('.xp-collapsible-toggle');
                    if (collToggle) {
                    ...
                    toggleBtn.classList.add('active');
                    // desktop remains fixed but visually indicates active state
            ...
            // fallback: show the read-only preview popup (Settings preview still uses this)
            const isPreviewActive = tracker.dataset._previewActive === '1';
            if (!isPreviewActive) {
                previewDisabledPopup(true);
                tracker.dataset._previewActive = '1';
            ...
            // Leveling is ENABLED — normal toggle behavior
            tracker.classList.toggle('expanded');
            toggleBtn.classList.toggle('active');
    ...
    document.addEventListener('click', (e) => {
        // If preview popup is active, close it properly and restore the collapsed state
        if (tracker.dataset._previewActive === '1' && !tracker.contains(e.target) && !toggleBtn.contains(e.target)) {
            previewDisabledPopup(false);
            delete tracker.dataset._previewActive;
            toggleBtn.classList.remove('active');
...
function initializeLevelingSystem() {
    ...
    if (!isLevelingEnabled()) {
        // Remove any old separate popup
        const existing = document.querySelector('.disabled-leveling-popup');
        if (existing) existing.remove();

        // Render tracker with the same layout as the normal character tracker
        if (tracker) {
            tracker.innerHTML = `
                ...
                <div class="character-info-display">
                    <h4 id="character-title" style="color:#ff2a6d;">Enable Leveling</h4>
                    <div class="xp-progress" style="opacity:0.25;">
                        <div class="xp-bar-fill" id="xp-bar-fill" style="width:0%;"></div>
                    </div>
                    <p class="xp-text" style="color:#888;font-family:'Courier New',monospace;text-transform:none;letter-spacing:0;font-size:0.68rem;line-height:1.35;margin-top:4px;">to enjoy the full experience</p>
                </div>
                <button class="close-popup-btn" aria-label="Close" type="button" style="position:absolute;top:6px;right:6px;background:transparent;border:none;cursor:pointer;padding:2px;display:flex;align-items:center;justify-content:center;width:20px;height:20px;z-index:1;">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M18.7071 5.29289C19.0976 5.68342 19.0976 6.31658 18.7071 6.70711L6.70711 18.7071C6.31658 19.0976 5.68342 19.0976 5.29289 18.7071C4.90237 18.3166 4.90237 17.6834 5.29289 17.2929L17.2929 5.29289C17.6834 4.90237 18.3166 4.90237 18.7071 5.29289Z" fill="#ff2a6d"/>
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M5.29289 5.29289C5.68342 4.90237 6.31658 4.90237 6.70711 5.29289L18.7071 17.2929C19.0976 17.6834 19.0976 18.3166 18.7071 18.7071C18.3166 19.0976 17.6834 19.0976 17.2929 18.7071L5.29289 6.70711C4.90237 6.31658 4.90237 5.68342 5.29289 5.29289Z" fill="#ff2a6d"/>
                    </svg>
                </button>
            `;
            tracker.setAttribute('style', '');
            tracker.style.border = '1px solid #ff2a6d';
            tracker.className = 'character-tracker mobile-footer-xp collapsible expanded';

            const closeBtn = tracker.querySelector('.close-popup-btn');
            if (closeBtn) {
        ...
        if (tracker) {
            ...
            if (!tracker.querySelector('.character-avatar-display')) {
                ...
                tracker.style.maxWidth = '';
                tracker.classList.remove('collapsible','collapsed','expanded');
...
// Preview helper: render/restore the "disabled" collapsible popup without touching localStorage
function previewDisabledPopup(enable) {
    const tracker = document.getElementById('character-tracker');
    if (!tracker) return;

    if (enable) {
        // save original state so we can restore it
        if (!('_origHtml' in tracker.dataset)) {
            tracker.dataset._origHtml = tracker.innerHTML;
            tracker.dataset._origStyle = tracker.getAttribute('style') || '';
            tracker.dataset._origClass = tracker.className || '';
        }

        // Render a *preview* of the full leveling tracker (read-only) so users can see
        // the experience UI even when leveling is disabled. This does NOT enable leveling
        ...
        tracker.innerHTML = `
            ...
            <button class="xp-collapsible-toggle" aria-expanded="true" aria-label="Close preview" type="button">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 15l-6-6h12l-6 6z" fill="#ff2a6d"/></svg>
            </button>
        `;

        // Visuals: look like the normal tracker but indicate preview state
        tracker.style.background = 'rgba(26, 26, 46, 0.95)';
        tracker.style.border = '1px solid #ff2a6d';
        tracker.style.maxWidth = '380px';
        tracker.style.display = 'flex';
        tracker.style.visibility = '';
        tracker.style.pointerEvents = '';

        // Start expanded so this behaves like a pop-up preview
        tracker.classList.add('collapsible', 'expanded');
        tracker.classList.remove('collapsed');

        // populate values from saved progression (read-only preview)
        try {
            updateCharacterDisplay(getProgressionState());
        } catch (e) {
            // harmless fallback if elements are missing
        }

        // attach toggle listener for preview (guarded)
        if (!tracker.dataset.previewInit) {
            const toggle = tracker.querySelector('.xp-collapsible-toggle');
            const bodyElements = tracker.querySelectorAll('.character-avatar-display, .character-info-display');
            if (toggle) {
                toggle.addEventListener('click', (e) => {
                    ...
                    if (isOpen) {
                        ...
                        toggle.setAttribute('aria-label', 'Open preview');
                        if (svg) {
                            svg.style.transition = 'transform 0.18s ease';
                            svg.style.transform = 'rotate(0deg)';
                        }
                        bodyElements.forEach(b => b.setAttribute('aria-hidden', 'true'));
                    } else {
                        tracker.classList.remove('collapsed');
                        tracker.classList.add('expanded');
                        toggle.setAttribute('aria-expanded', 'true');
                        toggle.setAttribute('aria-label', 'Close preview');
                        if (svg) {
                            svg.style.transition = 'transform 0.18s ease';
                            svg.style.transform = 'rotate(180deg)';
                        }
                        bodyElements.forEach(b => b.setAttribute('aria-hidden', 'false'));
                    }
                });

                // set initial icon rotation for preview button
                const initialExpanded = toggle.getAttribute('aria-expanded') === 'true';
                const tSvg = toggle.querySelector('svg');
                if (tSvg) {
                    tSvg.style.transition = 'transform 0.18s ease';
                    tSvg.style.transform = initialExpanded ? 'rotate(180deg)' : 'rotate(0deg)';
                }
            }
            tracker.dataset.previewInit = '1';
        }
    } else {
        // restore original tracker
        if ('_origHtml' in tracker.dataset) {
            tracker.innerHTML = tracker.dataset._origHtml;
            tracker.setAttribute('style', tracker.dataset._origStyle || '');
            tracker.className = tracker.dataset._origClass || 'character-tracker';
            delete tracker.dataset._origHtml;
            delete tracker.dataset._origStyle;
            delete tracker.dataset._origClass;
            delete tracker.dataset.previewInit;
        } else {
            // fallback — re-run normal initializer
            initializeLevelingSystem();
        }

        if (isLevelingEnabled()) updateCharacterDisplay();
    }
}

// Insert a small "Preview Disabled Popup" button into Settings (if present)
function attachPreviewToggle() {
    const existing = document.getElementById('preview-leveling-popup');
    if (existing) return;

    const toggleBtn = document.getElementById('toggle-leveling-btn');
    if (!toggleBtn || !toggleBtn.parentElement) return;

    const previewBtn = document.createElement('button');
    previewBtn.id = 'preview-leveling-popup';
    previewBtn.type = 'button';
    previewBtn.textContent = 'Preview Disabled Popup';
    previewBtn.style.cssText = 'background:transparent;color:#fff;border:2px solid #666;padding:12px 18px;font-size:0.95rem;font-family:\'Orbitron\',sans-serif;border-radius:8px;cursor:pointer;margin-left:8px;';

    previewBtn.addEventListener('click', () => {
        const active = previewBtn.dataset.active === '1';
        if (active) {
    ...
    toggleBtn.parentElement.appendChild(previewBtn);
}
    ...
    if (hasChosenLeveling) {
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => {
                ...
                // attach preview toggle in Settings (dev helper)
                attachPreviewToggle && attachPreviewToggle();
            });
        } else {
            initializeLevelingSystem();
            // Update character display if leveling is enabled
